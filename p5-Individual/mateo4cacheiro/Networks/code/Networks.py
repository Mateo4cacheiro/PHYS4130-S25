...
class UndirectedGraph:
	...
	that the user does not inadvertently change the neighbor list."""
        LazyBums=self.connections[node]
        return LazyBums
...
def FindPathLengthsFromNode(graph, node):
    ...
    shellLvl=0
    nextshell = [node]
    while len(lengths) < len(graph.GetNodes()):
...
def FindAllPathLengths(graph):
    ...
    L=len(Nodes)
    Lengths={}
    #for each node, find distance to every other node 
    for i in range(1,L+1):
        Lengths[i]=FindPathLengthsFromNode(graph,i)
    
    """
    The above makes L by L dictionary that should be symetetric. I want to return a dictionary with only one of the halves. I want to exclude the diagonal. (all zeros and not meaningful.
    """

    PathLengths = {}
    for i in range(1, L):
        for j in range(i+1, L+1):
            PathLengths["from "+str(i)+" to "+str(j)]=Lengths[i][j]

    return(PathLengths)

def FindAveragePathLength(graph):
    """Averages path length over all pairs of nodes"""
    AllPaths=FindAllPathLengths(graph)
    SumOfPaths = sum(AllPaths.values())
    NumOfPaths= len(AllPaths)
    AverageLength=SumOfPaths/NumOfPaths
