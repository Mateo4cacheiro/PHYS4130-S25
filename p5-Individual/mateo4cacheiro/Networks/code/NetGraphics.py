...
def GenerateCircleGraphImageWithWeights(graph, imfile, 
    ...
    im = imBig.resize((windowSize,windowSize), Image.LANCZOS)
    im.save(imfile)
    return im
...
def DisplayCircleGraph(graph, L, Z, p, 
                       edge_weights=None, node_weights=None,
                       edge_scale=None, node_scale=None,
                       windowSize=800, 
                       dotscale=4., linescale=2,
    ...
    filename = tempfile.mktemp()  # make unique temporary filename in /tmp
    filename += ".png"
    GenerateCircleGraphImageWithWeights(graph, imfile='tmp-2/'+str(L)+'N_'+str(Z)+'E_'+str(p)+'p.png', 
...
def DrawTriangularNetworkSites(graph, nodelists=None, L=0, 
    ...
    im = imBig.resize((heightx,heighty), Image.LANCZOS)
    im.save(imfile)
    Display(imfile)
    return im
